...
def approve_request_view(request,pk):
    adminenquiry=forms.AdminApproveRequestForm()
    requestdetails = models.Request.objects.get(id = pk)
    customer = requestdetails.customer
    cutomerfullname = customer.get_name
    email = customer.user.username
    paymethod = requestdetails.paymentmethod
    print(email)
    if request.method=='POST':
        adminenquiry=forms.AdminApproveRequestForm(request.POST)
        if adminenquiry.is_valid():
            ...
            enquiry_x.save()
            try:
                otp_email(cutomerfullname,enquiry_x.cost,paymethod,email)
